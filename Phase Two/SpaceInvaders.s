...
MOVE_BULLET_UP FUNCTION
	...
	PUSH{R0-R12,LR}
	MOV R0,#0                     ;Counter to loop on the Goblin array
	ldr R8, =BULLET_MEMORY_X
	ldrh R1, [R8]	;X1
	ldrh R2,[R8,#2] ;X2
	ldrh R3,[R8,#4]	;X3
...
cmpX11	
	...
	PUSH{R0-R12}   ;CHECKING COLLISIONS START
...
CHECK_COLLISION1
	...
	CMP R0,#36	
	BEQ CHECKS1_FINISHED
	ADD R0,#2
	CMP R3,#0
	BEQ CHECK_COLLISION1
	CMP R2,R3
	BLT CHECK_COLLISION1
	CMP R2,R6
	BGT CHECK_COLLISION1
	CMP R5,R4
	BGT CHECK_COLLISION1
	; DELETE GREEN GOBLIN
	; R2, R5: STARTING X, Y OF SPRITE
	...
	STRH R11,[R9,R0]
	CMP R11,#0
	BNE SKIP_GOBLIN_DELETE_1
	BL DEL_GREEN_GOBLIN
	STRH R6,[R1,R0]
	
SKIP_GOBLIN_DELETE_1
	POP{R0-R12}
	POP{R0-R12}
	B set_X1_0
CHECKS1_FINISHED
...
CHECK_COLLISION2
	...
	ADD R6,R3,#34    ;X2 GOBLIN
	CMP R0,#36	
	BEQ CHECKS2_FINISHED
	ADD R0,#2
	CMP R3,#0
	BEQ CHECK_COLLISION2
	CMP R2,R3
	BLT CHECK_COLLISION2
	CMP R2,R6
	BGT CHECK_COLLISION2
	CMP R5,R4
	BGT CHECK_COLLISION2
	; DELETE GREEN GOBLIN
	; R2, R5: STARTING X, Y OF SPRITE
	...
	STRH R11,[R9,R0]
	CMP R11,#0
	BNE SKIP_GOBLIN_DELETE_2
	BL DEL_GREEN_GOBLIN
	STRH R6,[R1,R0]
	
SKIP_GOBLIN_DELETE_2
	POP{R0-R12}
	POP{R0-R12}
	B set_X2_0
...
CHECK_COLLISION3
	...
	ADD R6,R3,#34    ;X2 GOBLIN
	CMP R0,#36	
	BEQ CHECKS3_FINISHED
	ADD R0,#2
	CMP R3,#0
	BEQ CHECK_COLLISION3
	CMP R2,R3
	BLT CHECK_COLLISION3
	CMP R2,R6
	BGT CHECK_COLLISION3
	CMP R5,R4
	BGT CHECK_COLLISION3
	; DELETE GREEN GOBLIN
	; R2, R5: STARTING X, Y OF SPRITE
	...
	STRH R11,[R9,R0]
	CMP R11,#0
	BNE SKIP_GOBLIN_DELETE_3
	BL DEL_GREEN_GOBLIN
	STRH R6,[R1,R0]
	
SKIP_GOBLIN_DELETE_3
	POP{R0-R12}
	POP{R0-R12}
	B set_X3_0
CHECKS3_FINISHED
