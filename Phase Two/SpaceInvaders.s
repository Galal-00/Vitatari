...
INITLVL2
		...
		POP{R0-R12}
		ldr R1, =SPACE_X  	;Space_X = Starting x
		ldr R8, =SPACE_Y	;Space_Y = Starting y
		ldrh r2,[r1]
		ldrh r5 , [r8]
		BL DRAW_SPACESHIP
...
SpaceGLoopLVL2
		...
		MOV R5,R6
		CMP R5,#0
		BEQ STOP_LOSE			;Game over if Health = zero
...
cmpX11LV2
	...
	ldrh R11,[R9] ;BOSS HEALTH
	CMP R11,#0		;BOSS already dead
	BEQ LV2_CHECKS_FINISHED
	CMP R2,#85            		;BOSS X1
	BLT LV2_CHECKS_FINISHED
	CMP R2,#234          		;BOSS X2
	BGT LV2_CHECKS_FINISHED
	CMP R5,#118					;BOSS Y2
	BGT LV2_CHECKS_FINISHED	
	SUB R11,R11,#1
...
cmpX22LV2
	...
	ldrh R11,[R9] 	;BOSS HEALTH
	CMP R11,#0		;BOSS already dead
	BEQ LV2_CHECKS_FINISHED2
	CMP R2,#85            		;BOSS X1
	BLT LV2_CHECKS_FINISHED2
	CMP R2,#234          		;BOSS X2
	BGT LV2_CHECKS_FINISHED2
	CMP R5,#118					;BOSS Y2
	BGT LV2_CHECKS_FINISHED2
	SUB R11,R11,#1
	STRH R11,[R9]				;UPDATE AND STORE BOSS HEALTH
...
;	CMP R12 , #1
	...
	strh r5,[R8,#8]
	b set_X2_0LV2
LV2_CHECKS_FINISHED2
	CMP R5, #25
	BLE set_X2_0LV2
	LDR R10, =WHITE
	BL DRAW_S_BULLET			;Draw the bullet in new location
	strh r5,[R8,#8]				;UPDATE AND STORE Y2
	B cmpX33LV2
...
cmpX33LV2
	...
	ldrh R11,[R9] ;BOSS HEALTH
	CMP R11,#0		;BOSS already dead
	BEQ LV2_CHECKS_FINISHED3
	CMP R2,#85            		;BOSS X1
	BLT LV2_CHECKS_FINISHED3
	CMP R2,#234          		;BOSS X2
	BGT LV2_CHECKS_FINISHED3
	CMP R5,#118					;BOSS Y2
	BGT LV2_CHECKS_FINISHED3
	SUB R11,R11,#1
	STRH R11,[R9]				;UPDATE AND STORE BOSS HEALTH
	...
	POP{R0-R3}
	b set_X3_0LV2
LV2_CHECKS_FINISHED3
	CMP R5, #25
	BLE set_X3_0LV2
	LDR R10, =WHITE
	BL DRAW_S_BULLET			;Draw the bullet in new location
	strh r5,[R8,#10]				;UPDATE AND STORE Y1
	POP{R2,R5}
	B NoMoveLV2
